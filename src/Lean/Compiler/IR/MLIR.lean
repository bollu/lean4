...
--    doc (loc: Loc) := doc loc.line ++ ":" ++ doc loc.column
...
syntax mlir_type "->" mlir_type : mlir_type
syntax "mlir.i" numLit : mlir_type -- this rule causes problems!
...
macro_rules
  ...
  | `(mlir_type% mlir.i $x:numLit ) => `(MLIRTy.int $x)
  | `(mlir_type% $dom:mlir_type -> $codom:mlir_type) => `(MLIRTy.fn (mlir_type% $dom) (mlir_type% $codom))

def ty0 : MLIRTy := (mlir_type% ())
def tyi32 : MLIRTy := (mlir_type% mlir.i 32) -- TODO: how to keep no gap?
-- def tyi32' : MLIRTy := (mlir_type% i32) -- TODO: how to keep no gap?
def tysingle : MLIRTy := (mlir_type% (mlir.i 42))
def typair : MLIRTy := (mlir_type% (mlir.i 32, mlir.i 64))
def tyfn0 : MLIRTy := (mlir_type% () -> ())
def tyfn1 : MLIRTy := (mlir_type% (mlir.i 11) -> (mlir.i 12))
def tyfn2 : MLIRTy := (mlir_type% (mlir.i 21, mlir.i 22) -> (mlir.i 23, mlir.i 24))
#print ty0
#print tyi32
#print typair
#print tyfn0
...
def opcall1 : Op := (mlir_op_call% "foo" (%x, %y) : (mlir.i 32, mlir.i 32) -> mlir.i 32)
#print opcall1
...
def bbstmt1 : BasicBlockStmt := 
  (mlir_bb_stmt% "foo"(%x, %y) : (mlir.i 32, mlir.i 32) -> mlir.i 32)
#print bbstmt1
def bbstmt2: BasicBlockStmt := 
  (mlir_bb_stmt% %z = "foo"(%x, %y) : (mlir.i 32, mlir.i 32) -> mlir.i 32)
#print bbstmt2
...
def bbop1 : SSAVal × MLIRTy := mlir_bb_operand% %x : mlir.i 32
#print bbop1
...
def bb1NoArgs : BasicBlock := 
  (mlir_bb%
     ^entry:
     "foo"(%x, %y) : (mlir.i 32, mlir.i 32) -> mlir.i 32
      %z = "bar"(%x) : (mlir.i 32) -> (mlir.i 32)
      "std.return"(%x0) : (mlir.i 42) -> ()
...
def bb3MultipleArgs : BasicBlock := 
  (mlir_bb%
     ^entry(%argp : mlir.i 32, %argq : mlir.i 64):
     "foo"(%x, %y) : (mlir.i 32, mlir.i 32) -> mlir.i 32
      %z = "bar"(%x) : (mlir.i 32) -> (mlir.i 32)
      "std.return"(%x0) : (mlir.i 42) -> ()
...
def oprgn0 : Op := (mlir_op_call%
 "func"() ( {
  ^bb0(%arg0: mlir.i 32, %arg1: mlir.i 32):
    %x = "std.addi"(%arg0, %arg1) : (mlir.i 32, mlir.i 32) -> mlir.i 32
    "std.return"(%x) : (mlir.i 32) -> ()
  }) : () -> ()
)
#print oprgn0
...
def attrVal1Ty : AttrVal := mlir_attr_val% (mlir.i 32, mlir.i 64) -> mlir.i 32
#print attrVal1Ty
...
def attr1Type : Attr := (mlir_attr% type = (mlir.i 32, mlir.i 32) -> mlir.i 32)
#print attr1Type
...
def opattr0 : Op := (mlir_op_call%
 "foo"() { sym_name = "add", type = (mlir.i 32, mlir.i 32) -> mlir.i 32 } : () -> ()
)
#print opattr0
...
private def opRgnAttr0 : Op := (mlir_op_call%
 "module"() (
 {
  ^entry:
   "func"() (
    {
     ^bb0(%arg0: mlir.i 32, %arg1: mlir.i 32):
      %zero = "std.addi"(%arg0 , %arg1) : (mlir.i 32, mlir.i 32) -> mlir.i 32
      "std.return"(%zero) : (mlir.i 32) -> ()
    }){sym_name = "add", type = (mlir.i 32, mlir.i 32) -> mlir.i 32} : () -> ()
   "module_terminator"() : () -> ()
 }) : () -> ()
)
#check opRgnAttr0
