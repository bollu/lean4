...
function compile_lean_llvm_backend {
    ...
    $CLANGPP -O0 -c -emit-llvm "$f.linked.bc" -o "$f.linked.O3.bc" || fail "Failed to optimize $f into a bitcode file and build an object file "
    $CLANGPP -c "$f.linked.O3.bc" -o "$f.linked.O3.o" || fail "Failed to optimize $f into a bitcode file and build an object file "
    leanc -g -o "$f.out" "$@" "$f.linked.O3.o" || fail "Failed to link object file '$f.linked.bc.o'"
    set +o xtrace
}
